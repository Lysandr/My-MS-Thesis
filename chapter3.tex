...
\subsection{Convexifying the Alignment Constraint}
Additionally, the convexity of the constraint proposed in \ref{alignconstraint} is not immediately clear. After analysis, it is shown to be non-convex. This is a downside to using MRP attitude coordinates over quaternions: the quaternion expression is exactly convex in the original form. Performing the first order Taylor linear approximation, we have the convexified form of this constraint:
%
\begin{align*}
    \frac{16\,\hat{\sigma}_2\,\left(\hat{\sigma}_2-\sigma_{2}\right)\,\left(\mathrm{\norm{\hat{\boldsymbol{\sigma}}}}+1 -2 (\hat{\sigma}_2^2 + \hat{\sigma}_3^2)\right)}{{\left(\mathrm{\norm{\hat{\boldsymbol{\sigma}}}}+1\right)}^3}-\frac{32\,\hat{\sigma}_1\,\left(\hat{\sigma}_1-\sigma_{1}\right)\,\left(\hat{\sigma}_2^2+ \hat{\sigma}_3^2\right)}{{\left(\mathrm{\norm{\hat{\boldsymbol{\sigma}}}}+1\right)}^3}
    -\frac{8(\hat{\sigma}_2^2+\hat{\sigma}_3^2)}{{\left(\mathrm{\norm{\hat{\boldsymbol{\sigma}}}}+1\right)}^2}
    \\ +\frac{16\,\hat{\sigma}_3\,\left(\hat{\sigma}_3-\sigma_{3}\right)\,\left(\mathrm{\norm{\hat{\boldsymbol{\sigma}}}}+1-2(\hat{\sigma}_2^2+\hat{\sigma}_3^2)\right)}{{\left(\mathrm{\norm{\hat{\boldsymbol{\sigma}}}}+1\right)}^3}+1 \geq \cos\psi_{max}
\end{align*}
We shall refer to this constraint as $\Psi (\boldsymbol{\sigma},\hat{\boldsymbol{\sigma}}) + 1 \geq \cos \psi_{max}$ going forward.

\subsection{Convexifying the Minimum Thrust Constraint}
With this done, we tackle the last source of non-convexity: the non-zero lower bound to our actuator thrust. Let us define an $\mathbb{R}^3 \rightarrow \mathbb{R}$ mapping function: $g(\mathbf{u}(\tau)) = F_{min} - \norm{\mathbf{u}(\tau)}_2 \leq 0$. Taleing the first orde Tayloar series linear approximation, we have the following convexified constraint formulation:
%
\begin{align}
...
WRITE A BUNCH MORE HERE ABOUT THE MODIFICATIONS TO THE OBJECTIVE FUNCTION
...
\end{align}
Given that we know the domain of the angle differences is constrained to a positive semidefinite arena of operation, we can perform a small angle approximation where $\mathbf{u}_k^T\mathbf{u}_{k+1} \leq u_k u_{k+1}\frac{\eta}{K}\dot{\delta}_{\text{TVC}_{max}}$.

THIS SHIT IS NONCONVEX WHAT SHOULD I TRY TO DO HERE???
...
%
\cos \psi_{max} &\leq 1 + \Psi (\boldsymbol{\sigma}^i,\boldsymbol{\sigma}^{i-1}) \\
%
|| \boldsymbol{\sigma}_{\mathcal{B/N}_k}||_2 & \leq 1 \\
%
...
\begin{align*}
F_{min} \leq \Xi_k^i\mathbf{u}_k^i, \quad \quad
